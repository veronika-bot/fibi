...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { answer, timeTaken } = parsed.data
    ...
    if (!task) return applyCors(req, NextResponse.json({ error: 'Task not found' }, { status: 404 }))
    ...
    if (progress.attempts >= 3) {
      return applyCors(req, NextResponse.json({ error: 'Max attempts reached', status: 'locked' }, { status: 403 }))
    }
    ...
    return applyCors(req, NextResponse.json(response))
  } catch (err) {
